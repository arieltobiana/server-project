...
(function($){
  ...
  QuickSelect = function($input_element, options){
    ...
    $input_element.attr('aria-autocomplete', 'list'); //judith
    var explID = $input_element.attr('id') + "_quickSelectExpl";
    $input_element.attr('aria-describedby', explID); //judith
    var resultsID = $input_element.attr('id') + "_results";
    $input_element.attr('aria-activedescendant', resultsID);//judith

    $quickSelectExpl = $('<span class="visuallyHiden" id="' + explID + '">שדה זה מכיל רשימת הצאות מילוי, עם תחילת הקלדה תוכל לבחור בין האפשרויות עי שימוש בחיצים, לבחירה יש ללחוץ על enter</span>');//judith
    $('body').append($quickSelectExpl);
    ...
    // Create the list DOM
      $results_list = $('<div class="'+options.resultsClass+'" id="'+ resultsID +'" tabindex="0"  style="display:block;position:absolute;z-index:9999;"></div>').hide();//judith
      ...
      $results_mask = $('<iframe />');
      $results_mask.css({border:'none',position:'absolute',display:'none'});
    	if(options.width>0){
    ...
    // Set up all of the methods
      self.getLabel = function(item){
		...
		var lbl = item[this.options.labelMember];
        return lbl;
      };
        ...
        return item.values || (item.value ? [item.value] : (typeof(item)==='string' ? [item] : item)) || []; // hash:item.values || item.value; string:item; array:item[1..end]
     	...
     	var moveSelect = function(step_or_li){
     	  ...
     	  if(typeof(step_or_li)==="number") activeSelection = activeSelection + step_or_li;
    		...
    		else if (activeSelection >= lis.length) activeSelection = lis.length - 1; //judith change size to length

    		lis.removeClass(options.selectedClass);
    		$(lis[activeSelection]).addClass(options.selectedClass);

    		if (last_keyCode == 38 || last_keyCode == 40) // JUDITH keyup or keydown - set the input value= selected so the screen reader can read it (also its more usefull)
    		$input_element.val($(lis[activeSelection]).text());
      ...
      var repopulate_items = function(items){
        ...
        for(var i=0; i<total_count; i++){
          ...
          $results_list.append(li);
    			$(li).text(options.formatItem ? options.formatItem.apply(self, [item, i, total_count]) : self.getLabel(item));
    			$(li).attr("role", "presentation"); //judith
      ...
      var show_results = function(){
      	...
      	var pos = $input_element.offset(),
            ...
            $lis = $('li', $results_list);
        if ($lis.length == 0) {
            if(options.onItemSelect)setTimeout(function(){ options.onItemSelect({item:null}); }, 1); // run the user callback, if set
        }
        else {
      	    // reposition
			var lft = pos.left;
			if ($(document.body).css('direction') == 'rtl' && options.width)
				lft -= options.width - $input_element.width();
      ...
      $input_element.keydown(function(e){
        ...
        switch(e.keyCode){
          ...
          case 40: // Down arrow - select next item in the drop-down
            ...
            break;
          case 9: //judith - tab close the selection, if not the tab will make a change on the page // Tab - select the currently selected, let the onblur happen
              // selectCurrent(); this was comented
              if(activeSelection > -1)
              {hideResultsNow();//judith
                  e.preventDefault();//judith
              }
            break;
  ...
  QuickSelect.formatItem = function(item) {
	if (!this.options.listFormat)
		return this.getLabel(item);
	var lbl = '';
	for(var i in this.options.listFormat) {
		var p = item[this.options.listFormat[i]];
		lbl += typeof(p) === 'undefined' ? this.options.listFormat[i] : p;
	}
	return lbl;
  };
  ...
  $.fn.quickselect = function(options, data){
	...
	options.labelMember	  = options.labelMember || 'label';
	options.valueMember	  = options.valueMember || 'value';
	options.formatItem	  = options.formatItem || QuickSelect.formatItem;
